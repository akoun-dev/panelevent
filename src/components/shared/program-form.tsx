...
export function ProgramForm({ initialData, onSave, loading = false }: ProgramFormProps) {
  const [formData, setFormData] = useState<ProgramFormData>({
    ...
    programText: initialData?.programText || '',
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        {formData.hasProgram && (
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'text' | 'structured')}>
            ...
            <TabsContent value="text" className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="programText">Programme détaillé</Label>
                <Textarea
                  id="programText"
                  value={formData.programText || ''}
                  onChange={(e) => setFormData(prev => ({ ...prev, programText: e.target.value }))}
                  placeholder="Décrivez le programme de votre événement...
Exemple :
• 09:00 - Accueil des participants
• 09:30 - Ouverture et présentation
• 10:00 - Conférence principale
• 12:00 - Pause déjeuner
• 14:00 - Ateliers en parallèle
• 16:00 - Table ronde
• 17:30 - Cocktail réseautage"
                  rows={10}
                />
                <p className="text-xs text-muted-foreground">
                  Vous pouvez utiliser des puces (•), des tirets (-) ou toute autre mise en forme textuelle
                </p>
              </div>
            </TabsContent>
